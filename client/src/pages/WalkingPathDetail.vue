    ...
    <section class="detail_header">
      ...
      <p>
        ...
        <!-- <i></i>리뷰 <b>8</b>
            <i>|</i> 댓글 <b>22</b> -->
        <span
          class="detail_info_from_map"
          v-if="getDetail.mapList && getDetail.mapList.length > 0"
        >
          <i>|</i> 거리
          <b
            >{{
              getDetail.mapList[0].distance >= 1000
                ? (getDetail.mapList[0].distance / 1000).toFixed(1) + " k"
                : getDetail.mapList[0].distance
            }}m</b
          >
          <i>|</i> 시간
          <b>{{
            getDetail.mapList[0].time >= 60
              ? (getDetail.mapList[0].time / 60).toFixed(0) +
                " 시간" +
                " " +
                (getDetail.mapList[0].time % 60) +
                " 분"
              : getDetail.mapList[0].time + " 분"
          }}</b>
        </span>
      </p>
      ...
      <div class="iac_wrapper">
        <article class="detail_info">
          <aside class="images">
            <figure class="viewer">
              <img
                v-if="getDetail.photosList && getDetail.photosList.length > 0"
                :src="
                  'http://localhost:8089/ex_images/' +
                  getDetail.photosList[0].imgName
                "
              />
              <img
                v-if="getDetail.photosList && getDetail.photosList.length === 0"
            ...
            <div class="img_list">
              <figcaption
                class="thumb"
                v-if="getDetail.photosList && getDetail.photosList.length > 0"
                v-for="photo in getDetail.photosList"
              >
                <img
                  :src="'http://localhost:8089/ex_images/' + photo.imgName"
                />
              </figcaption>
            </div>
          </aside>
...
const loadScript = function () {
  ...
  script.src =
    "//dapi.kakao.com/v2/maps/sdk.js?appkey=d753200077e444dd40df1e458903dfd3&autoload=false";
...
const fetchDetail = async () => {
  const response = await axios.get(
    `http://localhost:8089/walking-path/${WalkingPathId.value}`
  );
